...
function buildDashboardData(scanSetId) {
  ...
  const SCAN_DATA = scanSet.version === "v4" ? SCAN_DATA_V4 : SCAN_DATA_V3;
  ...
  for (const pk of Object.keys(PLATFORMS)) {
    if (activePlatforms.includes(pk) && SCAN_DATA[pk]) {
      ...
      platforms[pk] = {
        model: pk === "claude" ? "claude-sonnet-4-6" : pk === "perplexity" ? "sonar" : pk === "gemini" ? "gemini-2.5-flash" : "待掃描",
...
function MethodTab({ d, scanSetId }) {
  const isV4 = SCAN_SETS[scanSetId]?.version === "v4";
  return (
    <Section title="📖 方法論">
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: "18px 16px", fontSize: 13, lineHeight: 1.8, color: C.textMuted }}>
        ...
        <h3 style={{ color: C.text, fontSize: 15, marginBottom: 8, fontWeight: 600 }}>{isV4 ? "四軸 Prompt 策略" : "三層消費者漏斗"}</h3>
        {isV4 ? [
